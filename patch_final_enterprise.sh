...
echo "=== Iniezione Patch Smart Scroll e Adaptive Layout ==="
...
var src_default = {
  ...
  async fetch(request, env2, ctx) {
    ...
    try {
      ...
      if (url.pathname === "/api/ingest") return this.handle穩gest(request, env2);
...
<body class="h-screen w-screen flex flex-row overflow-hidden bg-[#09090b]">
  ...
  <!-- COLONNA 1: BARRA DELLE APPLICAZIONI (72px FISSI) -->
  ...
  <!-- COLONNA 2: INBOX LIVE LIST (350px FISSI) -->
  ...
  <!-- COLONNA 3: FINESTRA DI CONVERSAZIONE CORRENTE (FLUIDA - RESTRINGIBILE) -->
  ...
  <!-- PANNELLO 4: ANAGRAFICA LEAD (300px BLOCCHETTO FISSO A DESTRA) -->
...
function renderM(p) { 
  try {
    ...
    // LOGICA DI CONTROLLO DEL CURSORE (ANTI-TRAP SCROLL-DOWN)
    var isAtBottom = (msgA.scrollHeight - msgA.scrollTop - msgA.clientHeight) < 150;
    ...
    // Esegui lo scroll in fondo SOLO alla prima apertura o se l'utente si trovava già giù
...
npx wrangler deploy
echo "=== Deploy Completato ed Errori Risolti ==="
