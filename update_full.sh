...
echo "=== Compilazione SMR OS V46 con WhatsApp Markdown Parser ==="
...
// Funzione interna di formattazione nativa per i canali Meta
function formatWhatsAppText(text) {
  if (!text) return "";
  // Converte il grassetto da **text** a *text* per l'applicazione WhatsApp
...
var src_default = {
  ...
  async fetch(request, env2, ctx) {
    ...
    try {
      ...
      return new Response("SMR OS ULTRA TIER ACTIVE");
    } catch (err) {
  ...
  async saveLog(p, obj, env2, statusObj) {
    ...
    // Memory Optimization: Se l'oggetto contiene un payload immagine base64 esteso, taglia la stringa per preservare la RAM di KV
  ...
  async handleWidget(request, env2, ctx) {
    ...
    if (analytics) {
      await this.saveLog(channelId, { from: "system", text: `👁️ Visitatore dalla pagina: ${analytics.url}`, timestamp: Date.now() }, env2, st);
    }
    let msgText = text || "";
    let mediaId = null;
    let mediaType = null;
  ...
  async handleWhatsApp(m, name, env2, ctx) {
    ...
    // Esecuzione del traduttore sintattico Markdown-to-WhatsApp prima dell'invio
  ...
  async handleReply(request, env2) {
    ...
    if (!phone.startsWith("web_")) {
      ...
      if (template) {
        ...
        payload.type = "text";
        payload.text = { body: formatWhatsAppText(text) }; // Sincronizzazione formattazione su invio manuale staff
      }
...
<head>
  ...
  <title>SMR OS - Ultra Premium</title>
  ...
  <style>
    ...
    @media (max-width: 767px) {
      ...
      .mobile-fullscreen { position: fixed; inset: 0; z-index: 50; background: #09090b; display: flex !important; }
    }
  </style>
</head>
<body class="h-screen w-full flex overflow-hidden bg-[#09090b]">

  <aside class="w-[72px] bg-[#121214] flex flex-col items-center py-6 gap-6 border-r border-[#222226] shrink-0">
  ...
  <aside id="chatListContainer" class="w-full md:w-[360px] bg-[#121214] border-r border-[#222226] flex flex-col shrink-0">
    <div class="p-5 border-b border-[#222226]">
      <div class="flex justify-between items-center mb-4">
        <h2 class="font-bold text-xl tracking-tight text-white">Conversazioni</h2>
        <span class="bg-emerald-500/15 text-emerald-400 px-2 py-0.5 rounded text-xs font-semibold border border-emerald-500/30">Live</span>
      </div>
  ...
  <main id="chatAreaContainer" class="flex-1 flex flex-col relative bg-[#09090b] mobile-hide">
    ...
    <header id="chatH" class="h-[76px] border-b border-[#222226] px-6 flex items-center justify-between hidden bg-[#121214] z-10">
      <div class="flex items-center gap-4 min-w-0">
        ...
        <div class="min-w-0">
          ...
          <p id="hDetails" class="text-xs text-zinc-400 mt-0.5 truncate"></p>
        </div>
      </div>
    ...
    <div id="compWrapper" class="hidden flex-col border-t border-[#222226] bg-[#121214] p-4">
      <div id="waNotice" class="hidden bg-amber-500/10 border border-amber-500/20 px-4 py-2 rounded-lg text-xs text-amber-400 font-medium mb-3 items-center justify-between">
        <span>⚠️ Limite 24h WhatsApp superato. Invia un template approvato per riattivare la sessione:</span>
      ...
      <form id="comp" class="flex gap-3 items-end">
        ...
        <button type="submit" id="sendBtn" class="bg-blue-600 hover:bg-blue-700 text-white px-5 py-3 rounded-xl font-bold transition flex items-center justify-center shadow-lg shadow-blue-600/10 text-sm">Invia</button>
      </form>
    </div>
  </main>
  ...
  <div id="tZone" class="hidden absolute inset-0 bg-black/60 backdrop-blur-md z-50 flex items-center justify-center p-4">
    <div class="bg-[#121214] border border-[#222226] rounded-2xl p-6 w-full max-w-2xl shadow-2xl">
      <div class="flex justify-between items-center mb-4">
        <div>
          <h2 class="text-lg font-bold text-white">🧠 SMR Vault - Addestramento IA</h2>
          <p class="text-xs text-zinc-400 mt-0.5">Inserisci nuove informazioni nel cervello algoritmico del bot</p>
        </div>
        <button onclick="hideT()" class="text-zinc-500 hover:text-white bg-[#1c1c1f] p-1.5 rounded-full w-8 h-8 flex items-center justify-center transition">✕</button>
      </div>
      ...
      <div id="t-content-text" class="block"><textarea id="tText" class="w-full h-40 border border-[#2d2d34] bg-[#1c1c1f] p-4 rounded-xl text-white text-sm outline-none focus:border-blue-500 resize-none" placeholder="Inserisci specifiche tecniche, regole o cataloghi..."></textarea></div>
      <div id="t-content-link" class="hidden"><input type="url" id="tLink" placeholder="https://example.com/scheda-tecnica" class="w-full border border-[#2d2d34] bg-[#1c1c1f] p-3 rounded-xl text-white text-sm outline-none focus:border-blue-500"></div>
      ...
      <div class="mt-6 flex justify-end gap-3 text-sm">
        <button onclick="hideT()" class="px-4 py-2 font-semibold text-zinc-400 hover:text-white transition">Annulla</button>
        <button onclick="saveOmni()" id="tBtn" class="bg-blue-600 hover:bg-blue-700 text-white px-5 py-2 rounded-xl font-bold transition shadow-md">Salva nel Vault</button>
      </div>
    </div>
  </div>
...
function playNotificationSound(type) {
    try {
        ...
        if (type === 'urgent') {
            ...
            gainNode.gain.setValueAtTime(0.15, audioCtx.currentTime); oscillator.start();
            oscillator.stop(audioCtx.currentTime + 0.15);
        } else {
            oscillator.type = 'sine'; oscillator.frequency.setValueAtTime(587.33, audioCtx.currentTime);
            gainNode.gain.setValueAtTime(0.1, audioCtx.currentTime); oscillator.start();
...
window.addEventListener('focus', () => { document.title = "SMR OS - Ultra Premium"; });
...
async function forceSync() {
    try { await fetch('/api/chats?force=true'); location.reload(); } catch(e) {}
}
...
function renderL() { 
  try {
    ...
    filtered.forEach(c => {
      ...
      htmlContent += '<div class="flex mt-2 items-center"><span class="text-[9px] px-2 py-0.5 rounded font-bold tracking-wide ' + tagClass + '">' + tagText + '</span>' + (c.manual ? '<span class="text-[9px] font-bold ml-2 px-1.5 py-0.5 rounded bg-zinc-800 text-zinc-300">👤 OPERATORE</span>' : '') + '</div></div></div></div>';
    });
...
function renderM(p) { 
  try {
    ...
    document.getElementById('hDetails').innerText = (c.email ? c.email + ' • ' : '') + (c.channel==='whatsapp' ? '+'+c.phone : 'Widget Web Site') + ' • ' + lastTime;
    ...
    if (isWhatsApp24hExpired) { waNotice.classList.remove('hidden'); waNotice.classList.add('flex'); msgInput.disabled = true; msgInput.placeholder = "Scegli un template approvato..."; } 
    else { waNotice.classList.add('hidden'); waNotice.classList.remove('flex'); msgInput.disabled = false; msgInput.placeholder = "Scrivi una risposta tecnica..."; }

    var tb = document.getElementById('toggleBtn'); var rb = document.getElementById('resolveBtn');
    if(c.manual) { 
      tb.innerHTML = "🤖 Riattiva Bot";
      tb.className = "px-3 py-1.5 rounded-lg text-xs font-bold text-zinc-300 bg-zinc-800 hover:bg-zinc-700 border border-[#2d2d34] transition"; rb.classList.remove('hidden');
    } else { 
      tb.innerHTML = "👤 Prendi Controllo";
      tb.className = "px-3 py-1.5 rounded-lg text-xs font-bold text-white bg-blue-600 hover:bg-blue-700 transition"; rb.classList.add('hidden');
    }
    ...
    msgs.forEach(m => { 
      ...
      if (m.from === 'system') { html += '<div class="flex justify-center mb-3 w-full"><div class="msg-system break-words">'+m.text+'</div></div>'; return; }
      ...
      var mediaHtml = '';
      if (m.mediaId) {
        if (m.mediaType === 'image') { mediaHtml = '<a href="/api/media/' + m.mediaId + '" target="_blank"><img src="/api/media/' + m.mediaId + '" class="max-w-xs rounded-xl mb-2 border border-[#2d2d34] hover:opacity-90 transition"></a>'; } 
        else { mediaHtml = '<a href="/api/media/' + m.mediaId + '" target="_blank" class="flex items-center gap-2 bg-[#1c1c1f] p-3 rounded-lg text-xs mb-2 font-bold text-blue-400 border border-[#2d2d34] hover:bg-[#27272a] transition truncate">📎 '+(m.fileName||'Documento')+'</a>'; }
      }
      html += '<div class="flex flex-col '+(isRight ? 'items-end' : 'items-start')+' mb-2 w-full"><span class="text-[10px] text-zinc-500 mb-1 px-1">'+sender+' · '+dObj.toLocaleTimeString('it-IT', {hour:'2-digit', minute:'2-digit'})+'</span><div class="msg-bubble '+bubbleClass+'">'+mediaHtml+'<div class="whitespace-pre-wrap">'+(m.text||'')+'</div></div></div>';
    }); 
...
function openChatMobile(phone) {
  curr = phone; document.body.classList.add('mobile-view-chat');
  document.getElementById('chatAreaContainer').classList.remove('mobile-hide');
  fetchChatDetailAndRender(phone);
}

function closeChatMobile() {
  curr = null; document.body.classList.remove('mobile-view-chat');
  document.getElementById('chatAreaContainer').classList.add('mobile-hide');
...
async function sendNote() { var t = prompt("Inserisci promemoria / nota interna nascosta:"); if (!t || !curr) return; await fetch('/api/reply', {method: 'POST', body: JSON.stringify({phone: curr, text: t, isNote: true})}); load(); }
...
echo "Esecuzione deploy definitivo dell'architettura omnicanale..."
npx wrangler deploy

echo "=== SMR OS V46 Online ==="
