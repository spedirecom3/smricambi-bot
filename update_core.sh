...
echo "=== Compilazione SMR OS V47 TIER ENTERPRISE ==="
...
<head>
  ...
  <style>
    ...
    @media (max-width: 1023px) { .right-panel-hide { display: none !important; } }
...
<body class="h-screen w-full flex overflow-hidden bg-[#09090b]">
  ...
  <aside id="chatListContainer" class="w-full md:w-[350px] bg-[#121214] border-r border-[#222226] flex flex-col shrink-0">
  ...
  <main id="chatAreaContainer" class="flex-1 flex flex-col relative bg-[#09090b] border-r border-[#222226] mobile-hide">
    ...
    <div id="compWrapper" class="hidden flex-col border-t border-[#222226] bg-[#121214] p-4">
      <div id="waNotice" class="hidden bg-amber-500/10 border border-amber-500/20 px-4 py-2 rounded-lg text-xs text-amber-400 font-medium mb-3 items-center justify-between">
        <span>⚠️ Limite 24h WhatsApp superato. Invia un template:</span>
  ...
  <aside id="profileCard" class="w-[300px] bg-[#121214] flex flex-col right-panel-hide shrink-0 p-5 hidden">
    <h3 class="text-zinc-400 font-bold text-xs uppercase tracking-wider mb-4 border-b border-[#222226] pb-2">Dettagli Utente</h3>
    <div class="space-y-4">
      ...
      <div class="pt-2 border-t border-[#222226]"><label class="text-[10px] text-zinc-500 font-semibold block uppercase mb-1">Pagina Attiva (Live Track)</label><a id="pCardPage" href="#" target="_blank" class="text-xs text-blue-400 hover:underline block truncate">-</a></div>
    </div>
  </aside>
...
function renderM(p) { 
  try {
    ...
    // Iniezione Dati Real-Time Anagrafica Destra
    ...
    msgs.forEach(m => { 
      ...
      if (m.mediaId) {
        ...
        else { mediaHtml = '<a href="/api/media/' + m.mediaId + '" target="_blank" class="flex items-center gap-2 bg-[#1c1c1f] p-3 rounded-lg text-xs mb-2 font-bold text-blue-400 border border-[#2d2d34]">📎 '+(m.fileName||'Documento')+'</a>'; }
      }
...
function closeChatMobile() { curr = null; document.getElementById('chatAreaContainer').classList.add('mobile-hide'); document.getElementById('empty').classList.remove('hidden'); document.getElementById('profileCard').classList.add('hidden'); ['chatH','msgA','compWrapper'].forEach(id => document.getElementById(id).classList.add('hidden')); }
...
echo "Pubblicazione aggiornamento omnicanale..."
npx wrangler deploy
echo "=== Backend Aggiornato con Successo ==="
